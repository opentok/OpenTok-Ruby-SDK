...
module OpenTok
  ...
  class Client
    ...
    def list_archives(offset, count)
      ...
      response = self.class.get("/v2/partner/#{@api_key}/archive", query: query.empty? ? nil : query)
      case response.code
      when 200
        response
      when 403
    ...
    def stop_archive(archive_id)
      response = self.class.post("/v2/partner/#{@api_key}/archive/#{archive_id}/stop", headers: { 'Content-Type' => 'application/json' })
      case response.code
      when 200
        response
      when 400
    ...
    def delete_archive(archive_id)
      response = self.class.delete("/v2/partner/#{@api_key}/archive/#{archive_id}", headers: { 'Content-Type' => 'application/json' })
      case response.code
      when 204
        response
      when 403
