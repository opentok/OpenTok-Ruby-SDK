module OpenTok
  ...
  class Signals
    ...
    # @option options [String] :connection_id When a connId is specified, only that connection
    #   recieves a signal. Otherwise, the signal is sent to all clients connected to the session.
    #
    # @raise [ArgumentError]
    ...
    # @raise [OpenTokError]
    #   The client specified by the connId property is not connected to the session.
    # @raise [OpenTokError]
